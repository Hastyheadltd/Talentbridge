...
const JobDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (id && user) {
      const checkApplicationStatus = async () => {
        try {
          const response = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/applications/applied`, {
            params: {
              jobId: id,
              userId: user._id,
            },
  ...
  const handleApply = async () => {
    ...
    // If already applied, notify the user
    if (applied) {
      Swal.fire({
    ...
    // Check if the user is an approved freelancer
  ...
  return (
    <div className="max-w-[1170px] mx-auto mt-8 mb-16 flex justify-between items-start gap-6 ">
      ...
      <div className="mt-4 lg:w-[680px] me-5">
        <button className="bg-[#2670FF] text-white text-center px-2 py-1 capitalize rounded-full text-[13px] font-medium">
          {job.jobType}
        </button>
        <h1 className="text-[32px] leading-[40px] mt-1 font-bold text-[#3B3A40] mb-6">
          {job.title}
        </h1>
       
        <h2 className="border-[#EBEBEB] border-t pt-5 text-[20px] text-black font-bold">
          Job Overview
        </h2>
      ...
      <div className="bg-white w-[430px]">
        <div className="border border-[#EBEBEB] rounded-[16px] p-4">
          ...
          <div className="flex justify-between items-center my-3">
            <div>
              ...
              <h1 className="text-[20px] tracking-[-1px] font-bold">
                €{job?.salary}
              </h1>
          ...
          {user && (
            <button
              ...
              className={`w-full text-white text-[16px] flex justify-center items-center gap-3 mt-4 font-medium py-3 rounded-[15px] transition duration-300 ${
                applied
                  ? "bg-green-600"
                  : user?.approve && user?.role === "freelancer"
                  ? "bg-black"
        ...
        <div className="border mt-5 border-[#EBEBEB] rounded-[16px] p-4">
          <div className="flex items-center mb-4 gap-5">
            ...
            <div>
              <h2 className="text-[22px] font-medium text-black">
                {job?.userInfo?.companyName}
              </h2>
              <h2 className="text-[16px] text-black/70">
                {job?.userInfo?.location}
              </h2>
          ...
          <div className="my-4 border-t border-[#EBEBEB] pt-4">
            <h3 className="mt-1 mb-5 text-[16px] text-black font-bold">
              About Company
            </h3>
