...
export default function JobOpenings() {
  ...
  const VALID_SKILLS = [
    ...
    "Bootstrap", "Node.js", "Express.js", "NestJS", "Python", "Django", "Flask",
    "Java", "Spring Boot", "C#", ".NET", "PHP", "Laravel", "Ruby", "Ruby on Rails",
    "Go", "Rust", "SQL", "MySQL", "PostgreSQL", "MongoDB", "Firebase", "AWS",
    "Azure", "Google Cloud", "Docker", "Kubernetes", "GraphQL", "REST API",
    "Microservices", "Cybersecurity", "DevOps", "CI/CD", "Git", "Linux",
    "Machine Learning", "AI Development", "Data Science", "Big Data", "Blockchain",
    "Solidity", "Smart Contracts", "Game Development", "Unity", "Unreal Engine",
    "Swift", "Kotlin", "iOS Development", "Android Development", "Flutter",
    "React Native", "UI/UX Design", "Figma", "Adobe XD", "WordPress", "Webflow",
    "Shopify", "E-commerce", "SEO", "Content Writing", "Copywriting", "Marketing",
    "Social Media Management", "Project Management", "Scrum", "Agile",
  ...
  // 4) Count how many jobs have a particular language
  const getLanguageCount = (lang: string) =>
    jobs.filter((job) => job.languages?.includes(lang)).length;

  // 5) Distinct industries
  const industries = Array.from(
    new Set(jobs.map((job) => job.industry).filter(Boolean))
  ) as string[];

  // 6) Build skill counts
  ...
  // Filtered skills based on skillSearchTerm
  const filteredSkills = sortedSkills.filter((skill) =>
    skill.toLowerCase().includes(skillSearchTerm.toLowerCase())
  );

  // The chunk of skills currently visible
  ...
  useEffect(() => {
    ...
    if (selectedLanguages.length > 0) {
      tempJobs = tempJobs.filter((job) =>
        job.languages &&
        selectedLanguages.some((selLang) => job.languages.includes(selLang))
      );
    }
    ...
    // Skill search box (skillSearchTerm)
    if (skillSearchTerm) {
      tempJobs = tempJobs.filter((job) =>
        job.skills.some((js) =>
  ...
  // 9) Toggle skill in the selected skills array
  ...
  // 10) Toggle language in the selectedLanguages array
  ...
  // If no jobs or no filtered jobs, show message
  if (!jobs.length && !filteredJobs.length) {
    return (
      <div className="text-center flex items-center justify-center min-h-screen">
        Loading jobs or no jobs found...
      </div>
    );
  }

  return (
    <div className="max-w-[1336px] mx-auto mt-3 mb-16 ">
      ...
      {/* Main Content: Sidebar + Job Cards */}
      <div className="flex max-w-[1136px] mx-auto items-start gap-4 mt-6">
        {/* SIDEBAR */}
        <aside className="w-[273px] border border-primary rounded-[20px] p-3 h-fit">
          ...
          <div className="mb-4 border border-[#2367EA]/20 p-4 rounded-[20px]">
            <h3 className="font-semibold text-[18px] text-[#373940] mb-3">
              Release date
            </h3>
            <div className="flex flex-col space-y-3">
              ...
              <label className="inline-flex items-center justify-between cursor-pointer">
                ...
                <span className="bg-[#2E90FA24] w-5 h-5 text-center rounded-full flex justify-center items-center text-[10px] text-primary">
                  {jobs.filter((job) =>
                    checkJobRecency(job.createdAt, "last24hours")
                  ).length}
                </span>
              </label>
              {/* Last 7 days */}
              <label className="inline-flex items-center justify-between cursor-pointer">
                ...
                <span className="bg-[#2E90FA24] w-5 h-5 text-center rounded-full flex justify-center items-center text-[10px] text-primary">
                  {jobs.filter((job) =>
                    checkJobRecency(job.createdAt, "last7days")
                  ).length}
                </span>
              </label>
              {/* Last 30 days */}
              <label className="inline-flex items-center justify-between cursor-pointer">
                ...
                <span className="bg-[#2E90FA24] w-5 h-5 text-center rounded-full flex justify-center items-center text-[10px] text-primary">
                  {jobs.filter((job) =>
                    checkJobRecency(job.createdAt, "last30days")
                  ).length}
                </span>
              </label>
            </div>
                ...
                >
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      className="cursor-pointer"
                    />
                    <span className="text-text text-[14px] capitalize">
                      {etype}
                    </span>
                  </div>
                  <span className="bg-[#2E90FA24] w-5 h-5 text-center rounded-full flex justify-center items-center text-[10px] text-primary">
          ...
          {/* Region */}
          <div className="mb-4 border border-[#2367EA]/20 p-4 rounded-[20px]">
            <h3 className="font-semibold text-[18px] text-[#373940] mb-2">
              Region
            </h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {filteredJobs.map((job) => {
            // Example: 15% commission (if needed)
            const commission = job.salary ? (job.salary * 0.15).toFixed(2) : 0;

            return (
              <div key={job._id} className="w-[250px] mb-4">
                <div className="p-6 bg-[#031830] text-white border border-gray-700 rounded-2xl hover:shadow-md hover:shadow-blue-900 transition-all duration-300">
                  <h2 className="text-[16px] font-semibold mb-3 h-[50px] line-clamp-2">
                    {job.title}
                  </h2>
